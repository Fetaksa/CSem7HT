...
double[] FindAverageColumn(int[,] matrix)
{
    ...
    for (int j = 0; j < matrix.GetLength(1); j++)
    {
        ...
        double findAverage = sum / matrix.GetLength(1);
        avgNumbers[j] = Math.Round(findAverage, 2);
    }
...
int[,] arr = CreateMatrixRndInt(4, 4, -10, 10);
PrintMatrix(arr);
double[] average = FindAverageColumn(arr);
Console.WriteLine();
PrintArray(average);